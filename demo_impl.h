...
void init_window( int width, int height )
{
	...
	// OLD SDL CODE screen = SDL_SetVideoMode( width, height, 0, 0 );
	surface = SDL_CreateRGBSurface( SDL_SWSURFACE, width, height, 16, 0, 0, 0, 0 );
  screen = SDL_CreateWindow("NTSC Filter Demo", SDL_WINDOWPOS_UNDEFINED, SDL_WINDOWPOS_UNDEFINED, width, height, SDL_WINDOW_SHOWN);
	if ( !screen || !surface )
		fatal_error( "SDL initialization failed" );
}
...
void display_output( void )
{
	...
	if ( SDL_BlitSurface( surface, &rect, SDL_GetWindowSurface(screen), &rect ) < 0 )
